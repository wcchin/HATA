...
"""
#2019/11/21
this version is the network computation version, not matrix
"""
...
def a_node_ego(iter_item):
    ...
    if r == 0:
        ...
        g.node[n][SUCCESSORS] = list(g.successors(n))
        g.node[n][PREDECESSORS] = list(g.predecessors(n))
    else:
    ...
    #ego_pbar.update(1)
    # FIXME: review this; must return something, multiprocessing copied everythings so it is copied situation
    # TODO: replace pbar
    #print(res)
    #res = g.node[n]
    return #res
...
def generate_ego_graph(g, sp):
    ...
    for r in range(sp):
        ...
        node_res = pool.imap(a_node_ego, iterlist)
        node_res = list(node_res)
        ni = 0
        for n in g.nodes(data=False):
            res = node_res[ni]
            g.node[n][EGO_NETWORK_OUT + str(r)] = res[EGO_NETWORK_OUT + str(r)]
            g.node[n][EGO_NETWORK_IN + str(r)] = res[EGO_NETWORK_IN + str(r)]
            ni += 1
    """
    for r in range(sp):
        for n in g.nodes(data=False):
...
def bridge_or_bond(dginput, times=9, external=None, threads=4, kmax=None,
    ...
    dg = sorted(nx.weakly_connected_component_subgraphs(dginput, copy=False), key=len, reverse=True)[0]
...
"""
## modified from NetworkX connected_double_edge_swap
# @py_random_state(3)
def DG_connected_double_edge_swap(G, nswap=1, _window_threshold=3):
    ...
    while n < nswap:
        ...
        if window < _window_threshold:
            ...
            while wcount < window and n < nswap:
                ...
                if len(list(G.successors(u)))==0 or len(list(G.successors(x)))==0:
                    continue
                v = random.choice(list(G.successors(u)))
                y = random.choice(list(G.successors(x)))
